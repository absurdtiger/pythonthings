# Thanks, cyberstart moon base c03l02
...
# Create a socket. AF_INET means you're connecting to an IPv4 IP Address.
...
# CHALLENGE 1: Write a TCP Client that will connect to 127.0.0.1 on port 9990.
#              Your client must send "Knock, knock" to the server.
#              Then get the response, and print it out.

#this would be so much easier with pwntools

bleh = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
bleh.connect(('127.0.0.1', 9990))
bleh.send("Knock, knock".encode())
data = bleh.recv(1024)
print(data)
